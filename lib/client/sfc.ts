...
export function sfcParser(
  ...
  const scriptCompiled = compileScript(
    descriptor,
    { id: id + ":script" }
  )
  ...
  for (const [ index, style ] of descriptor.styles.entries()) {
    ...
    const url = [
      file,
      Object.entries(query).map((a) => a[1] ? a.join("=") : a[0]).join("&"),
    ].join("?")
    ...
    if (style.module) {
      ...
      const exposedName = typeof style.module === "string"
        ? style.module
        : "$style"
    ...
    if (Object.keys(cssModulesMap).length) {
      const mappingCode = Object.entries(cssModulesMap).map(([k,v]) => `"${k}": ${v}`).join(", ")
  ...
  const attachedPropsStr = Object.entries(attachedProps).map((a) => a.join(": ")).join(", ")
